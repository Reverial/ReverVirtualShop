			...
			if(args.length != 0) {
				if(args[0].toLowerCase().equals("gui")) {
					...
					if(VirtualShop.country.equals("TR")) {
						sender.sendMessage(f.c("&6VirtualShop &ev"+VirtualShop.plugin.getDescription().getVersion()+"&6 - 'Pikod' tarafndan yapld. Yardm almak iin &e'/vs help' &6yaznz."));
					}else {
						sender.sendMessage(f.c("&6VirtualShop &ev"+VirtualShop.plugin.getDescription().getVersion()+"&6 - created by 'Pikod' edited by 'DoreK'. Use &e'/vs help' &6to get commands."));
					}
					return true;
				}
			}else {
